...
char *D_DoomExeDir(void)
{
	...
	{
		base = malloc(strlen(lxdoom_dir) + 1);
		strcpy(base, lxdoom_dir);
		...
		if (handle)
		{
			...
			if (!strncmp(header.identification,"IWAD",4))
			{
				...
				*/
				...
				fread(fileinfo, 1, length*sizeof(filelump_t), handle);
				fclose(handle);
...
void D_DoomMainSetup(void)
{
	...
	default:
		...
		lprintf(LO_ALWAYS,"PS2Doom (built %s), playing: %s\n"
			"PS2Doom is released under the GNU General Public license v2.0.\n"
	...
	//jff 1/22/98 add command line parms to disable sound and music
	{
		...
		nomusicparm = 1; // DEV   nosound || M_CheckParm("-nomusic");
